...
def llama_hugging(data, model_name='llama-7b', gen_prompt=TruthfulQA_generation_prompt, w_file=None,
    ...
    for index, row in data.iterrows():
        ...
        while count < k:
            ...
            answer_new = answer.split('答案是29台计算机。')[-1].split('\n')
            for a in answer_new:
...
def eval():
    print('devices: {} is using...'.format(global_devices))
    # MGSM
    # altering = ['', '_ex_adjacent', '_ex_random_two']

    run_MGSM(lan='fr', model_name='llama-30b', altering='', max_new_tokens=100, evaluate=True)
    run_MGSM(lan='fr', model_name='llama-30b', altering='_ex_random_two', max_new_tokens=100, evaluate=True)
    run_MGSM(lan='fr', model_name='llama-30b', altering='_rotate_two_part', max_new_tokens=100, evaluate=True)
    run_MGSM(lan='fr', model_name='llama-30b', altering='_ex_adjacent', max_new_tokens=100, evaluate=True)
...
if __name__ == "__main__":
    ...
    # args.add_argument("--lan", type=str, default='zh')
    # # args.add_argument('--task', type=str, default='mgsm')
    # args.add_argument('--max_tokens', type=int, default=100)
    # args = args.parse_args()
    #
    # run_MGSM(lan=args.lan, model_name=args.model_name, altering=args.altering, max_new_tokens=args.max_tokens, evaluate=False)
