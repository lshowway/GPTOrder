        ...
        if pd.isnull(row[model_name]):
            ...
            while count < k:
                generate_ids = llama_model.generate(inputs.input_ids.to(llama_model.device),
                                                    max_new_tokens=max_new_tokens, top_k=10, top_p=0.9, do_sample=True, temperature=0.8, early_stopping=True,
                                                    use_cache=True
                                                    )
                answer = tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
                if 'mgsm' in w_file:
                    answer_new = answer.split('he answer is 29.')[-1].split('\n')
                    for a in answer_new:
                        if a.strip().startswith('A:') and 'he answer is' in a:
                            answer = a[2:]
                            k_answers.append(answer)
                            print('>>', answer)
                            count += 1
                            break
                elif 'TruthfulQA' in w_file:
                    answer_new = answer.split('held in Barcelona, Spain.')[-1].split('\n')
                    print("answer_new", answer_new)
                    for a in answer_new:
                        if a.strip().startswith('A:'):
                            ...
                            count += 1
                            break
            print('=================', time.time() - start, llama_model.device, model_name)
            data.loc[index, model_name] = str(k_answers) if len(k_answers) > 1 else k_answers[0]
            if (index + 1) % 5 == 0:
...
def run_truthfulQA(model_name='llama-7b', altering='_ex_random_two', max_new_tokens=50, k=1, evaluate=True):
    truthfuleQA_file = "../data/TruthfulQA/TruthfulQA{}.csv".format(altering)
    truthfulQA_data = load_TruthfulQA(truthfuleQA_file, samples_num=100)
    gen_prompt = TruthfulQA_generation_prompt
    truthfulQA_gen_file = "../data/TruthfulQA/{}_TruthfulQA{}.csv".format(model_name, altering)

    if not evaluate:
        if model_name in ['llama-7b', 'llama-13b', 'llama-30b', 'llama-65b']:
            llama_hugging(data=truthfulQA_data, model_name=model_name, gen_prompt=gen_prompt, w_file=truthfulQA_gen_file, max_new_tokens=max_new_tokens, k=1)
        ...
        new_data = load_TruthfulQA(truthfulQA_gen_file, samples_num=100)
...
def run_MGSM(lan='en', model_name='llama-7b', altering='_ex_random_two', temperature=0.8, k=3, max_new_tokens=None, evaluate=True):
...
def eval():
    ...
    # run_truthfulQA(model_name='llama-7b', altering='', max_new_tokens=50, k=1, evaluate=True)
    # run_truthfulQA(model_name='llama-7b', altering='_ex_random_two', max_new_tokens=50, k=1, evaluate=True)
    run_truthfulQA(model_name='llama-7b', altering='_rotate_two_part', max_new_tokens=50, k=1, evaluate=True)
    run_truthfulQA(model_name='llama-7b', altering='_ex_adjacent', max_new_tokens=50, k=1, evaluate=True)

    # run_MGSM(lan='fr', model_name='llama-7b', altering='', max_new_tokens=100, evaluate=True)
    # run_MGSM(lan='fr', model_name='llama-7b', altering='_ex_random_two', max_new_tokens=100, evaluate=True)
    # run_MGSM(lan='fr', model_name='llama-7b', altering='_rotate_two_part', max_new_tokens=100, evaluate=True)
    # run_MGSM(lan='fr', model_name='llama-7b', altering='_ex_adjacent', max_new_tokens=100, evaluate=True)
...
if __name__ == "__main__":
    ...
    # if args.task == 'mgsm':
    #     run_MGSM(lan=args.lan, model_name=args.model_name, altering=args.altering, max_new_tokens=100, evaluate=False)
    # elif args.task == 'TruthfulQA':
    #     run_truthfulQA(model_name=args.model_name, altering=args.altering, max_new_tokens=50, k=1, evaluate=False)